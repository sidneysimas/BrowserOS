...
The BrowserOS MCP server only listens on `127.0.0.1`, so by default only programs on the same computer can reach it. If your agent runs somewhere else — for example a [Hermes](https://github.com/NousResearch) or OpenClaw agent on a VPS controlling BrowserOS on your laptop — you need a reverse tunnel that forwards requests from the remote machine to the MCP server on your laptop.
...
All commands below use port `9239` — replace it with the port shown on your `chrome://browseros/mcp` settings page.
...
<Tabs>
  <Tab title="Tailscale (recommended)">
    ...
    ```bash
    claude mcp add --transport http browseros https://your-laptop.tailnet-name.ts.net/mcp --scope user
    ```
